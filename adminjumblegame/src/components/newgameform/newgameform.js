import React, { useState } from 'react';
import {useForm} from '@xanderite/react-hook-form';
import '../newgameform/newgameform.css'
import gameservice from '../../services/gameservice'
import Modal from 'react-modal';
const customStyles = {
    content : {
      top                   : '50%',
      left                  : '50%',
      right                 : 'auto',
      bottom                : 'auto',
      marginRight           : '-50%',
      transform             : 'translate(-50%, -50%)',
      padding               : '10vw'
    }
  };
export default function Newgameform(props){
    const [modalIsOpen,setIsOpen] = useState(false);
    const { register, handleSubmit, errors } = useForm();
    const {register:questions,handleSubmit:handlequestionsubmit,errors:qerrors}=useForm()
    const [question,setquestion]=useState([]);
    
   
    function closeModal(){
        setIsOpen(false)
    }
    
 
    const ongameSubmit = (data,e) => {
        console.log(data);
       data.start_time=Date.parse(data.start_time)/1000
       data.end_time=Date.parse(data.end_time)/1000
        data["questions"]=question
        data["creatoremail"]=props.user.currentuser.email
        console.log(data)
      gameservice.addgame(data).then(data=>{
          if(data.msg==="Game Added successfully"){
            setIsOpen(true)
        setTimeout(() => {
          setIsOpen(false)
          window.location.href="/home"
           
        }, 3000);
          }
        
    })
        
    }
    const onquestionsubmit = (data,e) => {
        e.preventDefault()
        
        data["shuffled_answer"] = data.answer.split('').sort(()=>(Math.random()-0.5)).join('');
        //console.log(data)
        setquestion(question.concat(data));
        console.log(question)
        e.target.reset()
    }

    return(
        <>
        <div className="frarea">
            <div className="topic uy">Add New Game</div>
            <div className="formarea flexrow">
                <div className="form1">
                    <span className="title">Game Details</span>
                    <form  className="flexcol" onSubmit={handleSubmit(ongameSubmit)}>
                    <div className="flexrow"><label>Gamename</label><span className="err"> *{errors.Game_Name && "This is required"}</span></div>
                    <input type="text" className="inp" placeholder="Game Name" name="Game_Name" ref={register({required: true, maxLength: 80})} />
                   
                    <div className="flexrow"><label>Game Description</label><span className="err">*{errors.Description && "This is required"}</span></div>
                    <input className="inp" type="text" placeholder="Description" name="Description" ref={register({required: true, maxLength: 100})} />
                    
                    <div className="flexrow"><label>Game Image link</label><span className="err">*{errors.Game_image && "This is required"}</span></div>
                    <input type="text" className="inp" placeholder="Game image link" name="Game_image" ref={register({required: true, minLength: 6, maxLength: 1200})} />
                    
                    <div className="flexrow"><label>Start time</label><span className="err">*{errors.start_time && "This is required"}</span></div>
                    <input type="datetime-local" className="inp" min={new Date().toISOString().split('T')[0]} placeholder="start time" name="start_time" ref={register({required:true})} />
                    
                    <div className="flexrow"><label>End time</label><span className="err">*{errors.end_time && "This is required"}</span></div>
                    <input type="datetime-local" className="inp"  placeholder="End time" name="end_time" ref={register({required:true})} />
                    <div className="flexrow"><label>Time Per Question<sub>(in seconds)</sub></label><span className="err">*{errors.question_time && "This is required"}</span></div>
                    <input type="number" className="inp"  placeholder="question time" name="question_time" ref={register({required:true})} />
                    
                    <div className="flexrow"><label>Theme color 1</label><span className="err">*{errors.theme_gradient1 && "This is required"}</span></div>
                    <input type="color" className="inp" placeholder="theme gradient1" name="theme_gradient1" ref={register({required:true})}/>
                    
                    <div className="flexrow"><label>Theme color 2</label><span className="err">*{errors.theme_gradient2 && "This is required"}</span></div>
                    <input type="color" placeholder="theme gradient2" className="inp" name="theme_gradient2" ref={register({required:true})}/>
                    
                    <input type="submit" className="butt lo" value="AddGame"/>
                    </form>
                </div>
                <div className="form2">
                    <span className="tit">Add Questions</span>
                    <form  className="flexcol" onSubmit={handlequestionsubmit(onquestionsubmit)}>
                    <div className="flexrow"><label>Question</label><span className="err"> *{qerrors.question && "This is required"}</span></div>
                    <input type="text" className="inp" placeholder="Question" name="question" ref={questions({required: true, maxLength: 80})} />
                    
                    <div className="flexrow"><label>Question Banner link</label><span className="err"> *{qerrors.question_banner && "This is required"}</span></div>
                    <input type="text" className="inp" placeholder="Banner image link" name="question_banner" ref={questions({required: true, maxLength: 100})} />
                    <div className="flexrow"><label>Correct Answer</label><span className="err"> *{qerrors.answer && "This is required"}</span></div>
                    <input type="text" className="inp" placeholder="Answer" name="answer" ref={questions({required: true, minLength: 6, maxLength: 1200})} />
                    
                    <input type="submit" className="butt lo" value="Add Questions" />
                    </form>
                    {question.length===0?"":
                    <div className="added">
                            <span className="tit">Added Questions</span>
                         <ul>
                         {question.length===0? "":question.map((q,index)=><li className="flexcol">
                              <span className="ques"><strong>{index+1}.Question:</strong>{q.question}</span>
                              <span className="ans"><strong>Answer:</strong>{q.answer}</span>
                         </li>)}
                     </ul>
                     </div>
                    }
                  
                </div>
            
            </div>
            
        </div>
        <Modal
          isOpen={modalIsOpen}
          onRequestClose={closeModal}
          style={customStyles}
          contentLabel="Example Modal">
              <img className="ki" src="data:image/png;base64,"/>
            <h1>game added successfully</h1>
        </Modal>
        </>
    )
}